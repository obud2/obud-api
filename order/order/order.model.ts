...
import { Plan, PlanOption, ReservationStatus } from '../plan/plan.model';
...
export class OrderItemDTO {
  ...
  validPlanInfo(plan: Plan) {
    ...
    if (this.price !== plan.price) {
      throw new Error(
        `클래스명 : ${this.lessonTitle}  ${this.startDate.replace('T', ' ')} ~ ${this.endDate.replace(
          'T',
          ' ',
        )} 의가격이 동일하지 않습니다.`,
      );
    ...
    if (remaining < this.reservationCount) {
      throw new Error(
        `클래스명 : ${this.lessonTitle}  ${this.startDate.replace('T', ' ')} ~ ${this.endDate.replace('T', ' ')} 자리가 모자랍니다.`,
      );
    }

    if (this.payOptionCount > 0) {
      if (!plan?.payOption?.price) {
        throw new Error(
          `클래스명 : ${this.lessonTitle}  ${this.startDate.replace('T', ' ')} ~ ${this.endDate.replace(
            'T',
            ' ',
          )} 에는 옵션이 없으므로 옵션인원을 추가할수 없습니다.`,
        );
      }
      if (this.payOptionCount > this.reservationCount) {
        throw new Error(
          `클래스명 : ${this.lessonTitle}  ${this.startDate.replace('T', ' ')} ~ ${this.endDate.replace(
            'T',
            ' ',
          )} 옵션의 인원이 예약인원을 초과할수 없습니다.`,
        );
      }
      if (plan.payOption?.price !== this?.payOption?.price) {
        throw new Error(
          `클래스명 : ${this.lessonTitle}  ${this.startDate.replace('T', ' ')} ~ ${this.endDate.replace('T', ' ')} 의 옵션가격이 다릅니다.`,
        );
      }
  ...
  setLessonInfo(lessonShort: LessonShort) {
    ...
    this.images = lessonShort.images;
  }
  async valid() {
...
export class UserInfo {
  ...
  constructor(headers: any) {
    ...
    this.hp =
      headers?.decoded['custom:hp'] === undefined || headers?.decoded['custom:hp'] === 'undefined' ? '' : headers?.decoded['custom:hp'];
  }
}
...
export class CancelDTO {
  ...
  constructor(data: any) {
    this.imp_uid = data.imp_uid;
    this.merchant_uid = data.merchant_uid;
    this.cancelAmount = data.cancelAmount;
    // this.reason = data.reason;
  }

  public static completeToCancel(data: any, amount: number) {
    const params: any = {
      ...data,
      cancelAmount: amount,
    };
    return new CancelDTO(params);
  }
}
